...
public class OuterDayChargeStrategy implements ITaxiChargeStrategy {

    @Override
    public double getStartingTaxiFare(TaxiChargeType chargeType, double distance) {
        return TaxiUnitPrice.OUTER_DAY_STARTING.getUnitPrice();
    }
    ...
    public double getThreeToTenKilometreTaxiFare(TaxiChargeType chargeType, double distance) {
        ...
        if (distance > THREE.getMaxDistance() && distance <= TEN.getMaxDistance()) {
            price = (distance - TaxiDistanceChargeFlag.THREE.getMaxDistance()) * TaxiUnitPrice.OUTER_DAY_BETWEEN_THREE_AND_TEN.getUnitPrice();
        } else if (distance > TEN.getMaxDistance()) {
            price = (TaxiDistanceChargeFlag.TEN.getMaxDistance() - TaxiDistanceChargeFlag.THREE.getMaxDistance()) * TaxiUnitPrice.OUTER_DAY_BETWEEN_THREE_AND_TEN.getUnitPrice();
        }
        return price;
    }
    ...
    public double getOverTenKilometreTaxiFare(TaxiChargeType chargeType, double distance) {
        if (distance > TEN.getMaxDistance()) {
            return (distance - TaxiDistanceChargeFlag.TEN.getMaxDistance()) * TaxiUnitPrice.OUTER_DAY_OVER_TEN.getUnitPrice();
        }
        return 0;
    }
